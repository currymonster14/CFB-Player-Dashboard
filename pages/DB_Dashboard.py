...
st.set_page_config(page_title="DB Dashboard", layout="wide")
...
def load_data():
    df1 = pd.read_excel("Utah Transfer Portal Master Sheet.xlsx", sheet_name=8)
    df2 = pd.read_excel("Utah TP Cross Check Board.xlsx", sheet_name=21)
...
data["PLAYER_DETAIL_LINK"] = data["player_id"].apply(
    lambda pid: f"DB_Path_Evaluations?player_id={pid}"
    )
